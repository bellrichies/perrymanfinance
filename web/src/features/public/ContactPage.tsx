...
export function ContactPage() {
  ...
  return <PublicLayout><Metadata title="Contact" seo={page.data?.data.seo} /><main id="content" tabIndex={-1} className="section-wrap"><Breadcrumb title="Contact" /><div className="grid gap-8 lg:grid-cols-[.82fr_1.18fr]"><section><p className="eyebrow">Contact</p><h1 className="mt-4 text-4xl font-semibold sm:text-5xl">{page.data?.data.title ?? 'Client Services'}</h1><div className="mt-5 text-lg text-slate-300"><RichText html={page.data?.data.excerpt} /></div><div className="mt-7 grid gap-4 text-sm"><article className="rounded-lg border border-white/10 bg-white/[0.04] p-5"><h2 className="font-semibold text-white">Contact information</h2><address className="mt-3 not-italic text-slate-300">12221 Merit Drive, Suite 1661<br />Dallas, TX 75251</address><a className="mt-3 inline-block font-semibold text-emerald-200" href="mailto:support@perrymanfinance.com">support@perrymanfinance.com</a></article><iframe title="Map showing Perryman Finance office near 12221 Merit Drive, Dallas, Texas" loading="lazy" className="h-72 w-full rounded-lg border border-white/10" src="https://www.google.com/maps?q=12221%20Merit%20Drive%2C%20Suite%201661%2C%20Dallas%2C%20TX%2075251&output=embed" /></div></section><section className="contact-form rounded-lg border border-slate-200 bg-white p-4 sm:p-5" aria-label="Contact form">
    ...
    {!mutation.isSuccess && <form noValidate onSubmit={(event) => void submit(event)} className="space-y-3" aria-busy={mutation.isPending}>
