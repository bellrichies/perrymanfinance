...
export function PhilosophyStatsSection() {
  const stats: { value: string; label: string; body: string }[] = [
    { value: 'Research', label: 'Market perspective', body: 'Global market, liquidity, securities, and digital asset themes framed for allocation and portfolio-service discussions.' },
    { value: 'Governance', label: 'Risk discipline', body: 'Opportunity reviews start with suitability, documentation, custody-model considerations, reporting expectations, and downside scenarios.' },
  ];
  return <section className="section-wrap reveal-section pt-10" aria-labelledby="philosophy-stats-title"><div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center"><div className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.04]"><img src={philosophyImage} alt="" aria-hidden="true" className="h-full min-h-[360px] w-full object-cover" loading="lazy" decoding="async" /></div><div><p className="eyebrow">Investment philosophy</p><h2 id="philosophy-stats-title" className="mt-3 text-3xl font-semibold text-white sm:text-4xl">Disciplined financial services for modern markets</h2><p className="mt-5 text-lg text-slate-300">PerrymanFinance helps clients evaluate investment services, securities exposure, wealth planning priorities, portfolio management, digital asset exposure, account-service expectations, and market information with a clear view of objectives and risk. Public pages support consultation and onboarding intake; regulated account, transaction, custody, and reporting processes require approved controls.</p><div className="mt-7 grid gap-4 sm:grid-cols-2">{stats.map((stat) => <article key={stat.label} className="stat-card compact-stat-card"><strong className="block text-3xl text-white">{stat.value}</strong><h3 className="mt-2 text-sm font-semibold uppercase text-emerald-100">{stat.label}</h3><p className="mt-2 text-sm text-slate-300">{stat.body}</p></article>)}</div></div></div></section>;
}
