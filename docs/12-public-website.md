...
Generated HTML contains visible public content and metadata, with canonical URLs, robots, Open Graph and Twitter fields. Sitemap generation excludes noindex routes and admin. Configure approved social images through SEO `open_graph.image`. No production origin or organizational legal identity is invented. Redirect automation and publication-triggered deployment remain SEO/operations follow-up work; add reviewed host redirects when changing published slugs.
