...
Publish pages with slugs `home`, `about`, `investment-solutions`, `digital-assets`, `wealth-management`, `how-it-works`, and `contact`. Publish metadata pages for `investments`, `insights`, and `faq` as well. Legal routes read published effective documents with matching slugs: `terms`, `privacy-policy`, `risk-disclosure`, `cookie-policy`. The shared legal template presents version/effective-date metadata, links the four legal documents, and includes related risk and consultation guidance. Legal content remains CMS-editable and versioned; local seed content is review-ready structure only, never approved production legal terms.

Sections use the existing `{type, content}` format. Content fields are `heading` (or `title`), `body` (sanitized rich text), `eyebrow`, and `items` containing `title`, `body`, and optional internal `href`. Links allow internal public paths only. The supported CMS types use shared hero, section heading, card, process, preview and CTA components. The `client_journey` type is used by `how-it-works`: it accepts ordered `items` for journey stages plus optional `checkpoints` and `preparation` item lists, all using the same title/body/internal-link shape. The `service_experience` type is used by the wealth-management and digital-assets pages: `variant`, `heading`, `eyebrow`, and `body` define the service introduction; `items`, `principles`, and `questions` provide editable, ordered topic lists with the same title/body/internal-link shape. Investment and insight previews query featured published records. No example investment performance or production legal wording is seeded.

Home uses these ordered CMS slots: `hero`, `positioning`, `services`, `philosophy`, `opportunities`, `risk`, and `cta`. Set `content.slot` explicitly or store sections in this order. Their types are respectively `hero`, `rich_text`, `service_grid`, `rich_text`, `investment_preview`, `rich_text`, and `cta`. Missing sections provide headings and empty API-backed lists without invented financial descriptions.

About uses the CMS slots `overview`, `services`, `approach`, `values`, `mission`, `why-perrymanfinance`, and `cta`. It explains the company as a financial-services platform for investment solutions, wealth-management context, securities-market perspective, and digital asset research. Its content must remain factual and risk-aware: do not add unverified history, leadership biographies, client figures, assets under management, performance, regulatory status, testimonials, awards, or claims of execution, custody, payment, or account functionality.
...
The local/development `PublicContentSeeder` creates the required public page records, ordered page sections, SEO metadata, public settings, investment categories, product and opportunity entries, article categories, tags, market insight articles, FAQs, and clearly marked legal placeholders so the frontend can load all documented routes after `php backend/bin/seed.php`. The seeded How It Works page explains a six-stage client journey, decision checkpoints, enquiry preparation, and the boundary between public education and formally approved processes. The seeded Wealth Management and Digital Asset Management pages use separately editable service journeys that cover relevant decision factors, review principles, consultation prompts, and scope boundaries. Seeded legal and financial-services copy is operationally realistic but not production-approved content and must be replaced or reviewed before release.
